...
export const ConfigProvider = ({ children }) => {
        ...
        const init = async () => {
            ...
            try {
                ...
                if (serverData) {
                    ...
                    if (local)
                        setConfigState(JSON.parse(local));
                }
            }
            catch (err) {
    ...
    useEffect(() => {
        ...
        if (!isAuthenticated)
            return; // Don't auto-save if not logged in
    ...
    return (_jsxs(ConfigContext.Provider, { value: { config, isLoaded, isAuthenticated, setConfig, saveConfig, login, logout }, children: [children, toast && _jsx(Toast, { message: toast.message, type: toast.type, onClose: () => setToast(null) })] }));
};
