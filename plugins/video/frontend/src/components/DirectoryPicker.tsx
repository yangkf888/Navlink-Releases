...
export function DirectoryPicker({ sourceId, isOpen, onClose, onSelect, initialPath = '/' }: DirectoryPickerProps) {
    ...
    return (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[100] backdrop-blur-sm" onClick={onClose}>
            <div
                className="bg-gray-950 rounded-2xl border border-gray-800 w-full max-w-xl max-h-[80vh] flex flex-col shadow-[0_0_50px_rgba(0,0,0,0.5)] overflow-hidden animate-in fade-in zoom-in duration-200"
            ...
            >
                ...
                <div className="flex items-center justify-between p-5 border-b border-gray-800 bg-gray-900/50">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="text-lg font-bold text-white leading-none mb-1">选择扫描目录</h3>
                            <p className="text-xs text-gray-500 flex items-center gap-1">
                                <span className="px-1.5 py-0.5 rounded bg-gray-800 border border-gray-700 text-[10px] uppercase">{getSourceLabel()}</span>
                            </p>
                        </div>
                    </div>
                    <button onClick={onClose} className="w-8 h-8 rounded-full flex items-center justify-center text-gray-500 hover:text-white hover:bg-white/10 transition-all">
                        <i className="fas fa-times"></i>
                    </button>
                </div>

                {/* 路径导航 */}
                <div className="flex items-center gap-3 px-5 py-3 bg-gray-900/30 border-b border-gray-800 backdrop-blur-md">
                    <button
                        onClick={handleGoUp}
                        disabled={currentPath === '/' || currentPath === ''}
                        className="w-8 h-8 flex items-center justify-center bg-gray-800 text-gray-400 hover:text-white hover:bg-gray-700 rounded-lg disabled:opacity-20 disabled:cursor-not-allowed transition-all"
                        title="返回上级"
                    >
                        <i className="fas fa-level-up-alt rotate-[-90deg]"></i>
                    </button>
                    <div className="flex-1 px-3 py-1.5 bg-black/40 border border-gray-800 rounded-lg text-sm text-blue-400 truncate font-mono shadow-inner shadow-black">
                        {currentPath || '/'}
                    </div>
                </div>
                        ...
                        <div className="grid grid-cols-1 gap-1">
                            {items.map((item, idx) => (
                                <div
                                    ...
                                    className="group flex items-center gap-4 px-4 py-3 rounded-xl cursor-pointer hover:bg-white/5 border border-transparent hover:border-gray-800 transition-all"
                                >
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <span className="text-gray-200 font-medium group-hover:text-white transition-colors block truncate">{item.name}</span>
                                        <span className="text-[10px] text-gray-600 font-mono truncate block">{item.path}</span>
                                    </div>
                                    <i className="fas fa-chevron-right text-gray-700 group-hover:text-gray-400 text-xs translate-x-[-10px] opacity-0 group-hover:translate-x-0 group-hover:opacity-100 transition-all"></i>
                                </div>
                            ))}
                        </div>
                    )}
                ...
                <div className="flex items-center justify-between p-5 border-t border-gray-800 bg-gray-900/50">
                    ...
                    <div className="flex gap-3">
                        <button
                            onClick={onClose}
                            className="px-5 py-2 text-sm bg-gray-800 text-gray-300 hover:text-white hover:bg-gray-700 rounded-xl font-medium transition-all"
                        >
                            取消
                        </button>
                        <button
