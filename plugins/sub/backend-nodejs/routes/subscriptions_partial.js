...
const { sendAllNotifications, sendBarkNotification, sendTelegramNotification, sendEmailNotification, sendWebhookNotification } = require('../services/notification');
...
router.get('/', async (req, res) => {
    try {
        getSubscriptionDAO().getAll((err, subscriptions) => { if (err) return res.status(500).json({ error: 'Failed to get subscriptions' }); res.json(subscriptions); });
        return;
...
router.get('/settings/notifications', async (req, res) => {
    try {
        getNotificationDAO().get((err, settings) => { if (err) return res.status(500).json({ error: 'Failed to get settings' }); res.json(settings || {}); });
        return;
        res.json(settings || {});
    } catch (error) {
