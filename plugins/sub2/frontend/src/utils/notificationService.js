 ...
 * 发送NotifyX通知
 */
export async function sendNotifyXNotification(title, content, apiKey, endpoint) {
    try {
        if (!apiKey) {
 ...
 * 发送Webhook通知
 */
export async function sendWebhookNotification(title, content, webhookUrl, method = 'POST') {
    try {
        if (!webhookUrl) {
 ...
 * 发送Bark通知 (iOS推送)
 */
export async function sendBarkNotification(title, content, deviceKey, server = 'https://api.day.app') {
    try {
        if (!deviceKey) {
 ...
 * 测试通知功能 - 统一入口
 */
export async function testNotification(platform, settings) {
    const testTitle = '🔔 订阅管理测试通知';
    const testContent = `这是一条测试通知消息。\n\n发送时间：${new Date().toLocaleString('zh-CN')}\n平台：${platform.toUpperCase()}`;
    const testMessage = `*${testTitle}*\n\n${testContent}`;
    switch (platform) {
        case 'telegram':
            return await sendTelegramNotification(testMessage, settings.telegram.botToken, settings.telegram.chatId);
        case 'notifyx':
            return await sendNotifyXNotification(testTitle, testContent, settings.notifyx.apiKey, settings.notifyx.endpoint);
        case 'webhook':
            return await sendWebhookNotification(testTitle, testContent, settings.webhook.url, settings.webhook.method);
        case 'bark':
            return await sendBarkNotification(testTitle, testContent, settings.bark.deviceKey, settings.bark.server);
        default:
