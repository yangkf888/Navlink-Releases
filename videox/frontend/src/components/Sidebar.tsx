...
export function Sidebar({
    ...
    const renderLiveSidebar = () => (
        <div className="w-full space-y-0.5 mt-2">
            <SidebarItem
                ...
                collapsed={collapsed}
            />
            {['bilibili', 'douyin'].map(platform => (
                <div key={platform} className="w-full">
                    <SidebarItem
                        icon={platform === 'bilibili' ? "fab fa-product-hunt" : "fab fa-tiktok"}
                        label={platform === 'bilibili' ? "B站" : "抖音"}
                        isActive={navParams.platform === platform}
                        onClick={() => {
                            if (!collapsed) toggleGroup(`live-${platform}`);
                            onNavigate('live', { platform });
                        }}
                        collapsed={collapsed}
                        hasChildren={liveSources.some(s => s.platform === platform)}
                        isExpanded={expandedGroups[`live-${platform}`] !== false}
                    />
                    {!collapsed && expandedGroups[`live-${platform}`] !== false && (
                        <div className="space-y-0.5">
                            {liveSources.filter(s => s.platform === platform).map(source => (
                                <SidebarItem
                                    key={source.id}
                                    icon=""
                                    dot={true}
    ...
    const renderNetdiskSidebar = () => (
        <div className="w-full space-y-0.5 mt-2">
            ...
            {netdiskSources.filter(s => s.enabled).map(source => {
                ...
                const isExpanded = expandedGroups[`netdisk-${source.id}`] !== false;
                return (
                    <div key={source.id} className="w-full">
                        <SidebarItem
                            ...
                            onClick={() => {
                                if (!collapsed && scanPaths.length > 0) toggleGroup(`netdisk-${source.id}`);
    ...
    const renderVideoSidebar = () => (
        <div className="w-full space-y-0.5">
            ...
            <div className="w-full mt-2">
                {!collapsed ? (
                    <div
                        className="w-full flex items-center px-4 py-3 text-sm font-medium text-secondary cursor-pointer hover:text-black dark:hover:text-primary transition-colors"
                    ...
                    <div className="flex justify-center py-2 text-secondary border-t border-border-color mt-2 pt-4">
                        <i className="fas fa-server text-xs" title="视频源"></i>
                    </div>
                )}
                {(isSourcesOpen || collapsed) && sources.map(source => (
                    <SidebarItem
                        key={source.id}
                        icon="fas fa-server"
                        label={source.name}
    ...
    const renderTvSidebar = () => {
        let displayGroups = groupedChannels;
        if (tvSearch) {
            displayGroups = {};
            Object.entries(groupedChannels).forEach(([group, channels]) => {
                ...
                if (filtered.length > 0) displayGroups[group] = filtered;
            });
        }
        const currentExpanded = tvSearch ? Object.keys(displayGroups).reduce((acc, g) => ({ ...acc, [g]: true }), {} as Record<string, boolean>) : expandedGroups;
                ...
                {!collapsed && (
                    <>
                        <div className="w-full mb-2">
                            ...
                            />
                            {expandedGroups['tv-sources'] !== false && tvSources.map(s => (
                                <SidebarItem
                                    key={s.id}
                        ...
                        <div className="px-3 mb-2">
                            <div className="relative">
                                <input
                                    type="text"
                                    className={`w-full text-xs rounded-full border py-1.5 pl-8 pr-3 focus:outline-none focus:border-blue-500 ${theme === 'dark' ? 'bg-secondary text-gray-200 border-border-color' : 'bg-gray-100 text-gray-800 border-gray-200'}`}
                                    placeholder="搜索频道..."
                                    value={tvSearch}
                        ...
                        <div key={group} className="w-full">
                            {!collapsed ? (
                                <>
                                    ...
                                    />
                                    {currentExpanded[group] && channels.map((ch, idx) => (
                                        <SidebarItem
                                            key={`${idx}-${ch.name}`}
                                            icon=""
                                            img={ch.logo}
    ...
    const renderMediaServerSidebar = () => (
        <div className="w-full space-y-0.5 mt-2">
            ...
            {mediaServers
                .filter(s => s.enabled && (isAdminAuthenticated || s.hidden !== 1))
                .map(server => {
                    ...
                    return (
                        <div key={server.id} className="w-full">
                            <SidebarItem
                                ...
                                onClick={() => {
                                    if (!collapsed && categories.length > 0) toggleGroup(`media-server-${server.id}`);
                                    onMediaServerChange?.(server.id);
                                    onNavigate('media_server', { mediaServerId: server.id });
                                }}
                                collapsed={collapsed}
    ...
    return (
        <div className={`flex flex-col h-full w-full select-none pb-safe transition-all duration-500 border-r glass-effect ${theme === 'dark' ? 'text-slate-300 border-border-color shadow-2xl' : 'text-slate-600 border-slate-200 shadow-xl'}`}>
            {renderHeader()}
            <div className={`flex-1 w-full min-w-full overflow-y-auto overflow-x-hidden sidebar-scrollbar scrollbar-overlay space-y-0.5 ${collapsed ? 'py-4' : 'py-2'}`}>
                ...
                {activeModule === 'tv' ? renderTvSidebar() :
                    activeModule === 'live' ? renderLiveSidebar() :
                        activeModule === 'netdisk' ? renderNetdiskSidebar() :
                            activeModule === 'media_server' ? renderMediaServerSidebar() :
                                renderVideoSidebar()}
            </div>
            <div className={`p-3 border-t space-y-1 ${theme === 'dark' ? 'border-border-color' : 'border-gray-200'}`}>
                {!isMobile && onToggleCollapse && (
                    <button
                        ...
                        className={`flex items-center w-full px-3 py-2 text-sm rounded-md transition-colors ${collapsed ? 'justify-center' : 'justify-start'} ${theme === 'dark' ? 'text-secondary hover:text-primary hover:bg-white/5' : 'text-secondary hover:text-gray-900 hover:bg-black/5'}`}
...
function SidebarItem({ icon, label, isActive, onClick, collapsed, hasChildren, isExpanded, level = 0, dot = false, img, status = 'none' }: SidebarItemProps) {
    const isLevel1 = level === 1;
    return (
        <div
            className={`group w-full flex items-center transition-all duration-300 text-sm cursor-pointer relative min-h-[46px] my-0.5 ${collapsed ? 'justify-center py-3 px-0' : `${isLevel1 ? 'pl-10 pr-4' : 'px-4'} py-2.5`} ${isActive ? 'active-brand-item text-white font-bold shadow-lg shadow-blue-500/30 z-10' : 'text-secondary dark:text-secondary hover:text-slate-900 dark:hover:text-slate-100 hover:bg-slate-100 dark:hover:bg-white/5'} ${!collapsed && isActive ? 'rounded-xl mx-2 w-[calc(100%-16px)]' : ''} ${collapsed && isActive ? 'rounded-lg mx-1 w-[calc(100%-8px)]' : ''}`}
            onClick={onClick}
            title={collapsed ? label : ''}
        >
            <div className={`flex items-center justify-center ${collapsed ? '' : 'w-8 -ml-1'} transition-transform group-hover:scale-110`}>
                {img ? (
                    <img src={img} className="w-4 h-4 object-contain opacity-80 group-hover:opacity-100 rounded-sm bg-white/10" onError={(e) => (e.target as HTMLImageElement).style.display = 'none'} />
                ) : dot ? (
            ...
            {isActive && !collapsed && <div className="absolute inset-0 bg-blue-400/10 blur-xl -z-10 rounded-full"></div>}
        </div>
    );
}
